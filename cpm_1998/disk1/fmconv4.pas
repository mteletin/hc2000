...
k:=0;while not(eof(f1))and(k<120)do begin
blockread(f1,mem[$5100+k*128],1);k:=k+1;end;
if b then begin rewrite(f2);b:=false;end;
blockwrite(f2,mem[$5100],k);end;
close(f2);close(f1);
rename(f2,s2);
end;end;
end;
...
var f1:file;
    ...
    c:char;
    r:boolean;
procedure trt(s1,s2:tip1;r:boolean);{1} begin
r:=true;assign(f1,s1);assign(f2,s2);reset(f1);rewrite(f2);
k:=0;while not(eof(f1)) do begin blockread(f1,mem[$b000],1);
for i:=1 to 128 do begin k:=k+1;if (k>9)or(k<=0) then
if k>=11 then write(f2,mem[$afff+k mod 128]);
end;end;close(f1);close(f2);end;
...
s2[12]:='I';s2[13]:='M';s2[14]:='G';s2:=space(s2);
       write('    To:');lowvideo;
       write(s2:15);
       highvideo;
if c<>'~' then begin read(kbd,c);
...
if c<>chr(27) then begin
  s2:=space(s2);if s2[2]<>':' then
  s2:=s1[1]+':'+s2;
  trt(s1,s2,r);if c<>chr(27) then l1:=1;
end;end;
if c<>chr(27) then
if (upcase(s2[1])='A')and(l1=1)then readf(drv[0]) else readf(drv[1]);
list(cv[0],0);list(cv[1],1);
...
var m:integer;l,h,r:byte;
    ...
    s:tip1;
begin
clrscr;s:=da[drv[cd]][a+y*(nmf+1)+1+x[cd] mod (nmf+1)];{2}
s[9]:='.';s:=space(s);
assign(f,s);reset(f);
for m:=1 to 6144 do begin
read(f,r);h:=m div 256;l:=m mod 256;
mem[$bfff]:=h;mem[$bffe]:=l;mem[$bffd]:=r;
inline($F3/$3E/$EE/$D3/$C5/$21/$00/$c0/$ED);
inline($5B/$FE/$BF/$19/$3A/$FD/$BF/$77/$3E/$EE/$D3/$C7/$FB);
end;close(f);read(kbd,c);
list(cv[0],0);
list(cv[1],1);
end;
...
if c=chr(13) then begin
 ...
 o:=o+chi(m[a+y*(nmf+1)+1+x[cd] mod (nmf+1)][i]);
  if o='SCR' then begin {apel procedura de incarcare COM }
                  write(#07);transf;end;
  if o='IMG' then playaud;

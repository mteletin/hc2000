...
ni:=64;while(t[x,ni]=' ')and(ni>1)do ni:=ni-1;
for y:=1 to ni do
write(f,t[x,y]);
...
while not eof(f) do
 BEGIN
  y:=1;
  readln(f,ln);a:=length(ln);
  for y:=0 to (a-1) div 64 do
    BEGIN
    if a>64 then b:=64 else b:=a;
    for lo:=1 to b do
      BEGIN
      t[x,lo]:=ln[lo+length(ln)-a];
      END;
    a:=a-b;
    x:=x+1;
    END;
...
initfile;savetxt;s:=s1;END
else if c1='D' then BEGIN
paper(cl[1]);ink(cl[6]);
initfile;loadtxt;s:=s1;initx;inity;END
...
END
else if c2='4' then BEGIN
for p:=0 to 3 do cl[p]:=0;
for p:=4 to 7 do cl[p]:=7;END
else if c2='2' then readcfg;
...
writeln;writeln('Personal Editor V7.0 (c) 1996-1997 TIM Software Power .');
...
END else
BEGIN for x:=0 to 7 do read(f,cl[x]);close(f);
border(cl[0]);iobord(cl[1]);paper(cl[2]);ink(cl[3]);
if cl[4]=1 then clrscr;END;

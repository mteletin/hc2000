...
s:string[66];begin
s:='';for k:=0 to yy-y do s:=s+'';
for k:=x to xx do begin gotoxy(y+1,k+1);write(s);end;
end;
...
gotoxy(2+b*27,1+i);
if sl[drv[b]][a+i]=chr(1) then lowvideo else highvideo;
...
if u>=0 then begin
  for i:=1 to u+1 do begin
  if sl[drv[b]][a+i+nmf+1]=chr(1) then lowvideo else highvideo;
...
if sl[drv[d]][a+xx+1]=chr(1) then lowvideo
else highvideo;
...
if sl[drv[d]][a+x[cd]+1]=chr(1) then lovideo
else highvideo;
...
k:=0;while not(eof(f1))and(k<70)do begin
user(v[cd]);
blockread(f1,mem[$5d00+k*128],1);k:=k+1;end;
if b then begin user(v[q]);rewrite(f2);b:=false;end;
user(v[q]);blockwrite(f2,mem[$5d00],k);end;
...
gotoxy(1,12);write('File:',o);
assign(ft,o);{$i-} reset(ft);{$i+}
...
o:='FMSMP'+chr(0)+'$';
for i:=1 to 7 do mem[$6400+i]:=ord(o[i]);
mem[$6400]:=5;
o:='USER 0'+chr(0)+'$';
...
writeln(ft,drv[1]);writeln(ft,nmf);close(ft);
user(v[cd]);halt;end;
...
'#':if nfd[drv[cd]]>0 then view
'$':begin end;
'%':if nfd[drv[cd]]>0 then begin copyf;
'&':if nfd[drv[cd]]>0 then begin movef;
'''':begin selusr(0);list(cv[0],0);end;
'(':if nfd[drv[cd]]>0 then begin erasef;
